...
object AnomalyDetector {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val rawData = sc.textFile("/Users/sbelur/work/kdd/ipfile.data")
  ...
  def clusteringTake1(rawData: RDD[String]): Unit = {
    ...
    /*(5 to 30 by 5).map(k => (k, clusteringScore(data, k))).
      foreach(println)
*/
    (100 to 160 by 10).map(k => (k, clusteringScore2(data, k))).
  ...
  def buildAnomalyDetector(
    ...
    println(distances.top(50).mkString(""))
    val threshold = distances.top(50).last
  ...
  def anomalies(rawData: RDD[String]) = {
    ...
    anomalies.take(30).foreach(println)
  }
